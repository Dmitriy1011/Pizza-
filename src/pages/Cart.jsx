...
function Cart() {
    ...
    const addedPizzas = Object.keys(items).map(key=> { //получаем от объекта ключи и items (0,1)
      return items[key].items[0]; //вытащим первый объект их массива
    }) 

    const onClearCart = () => {
      if(window.confirm('Вы действительно хотите очистить корзину?')) {
      dispatch(clearCart()); //dispatch отправляет метож clearCart в redux
    }
  };
  ...
  <path d="M11.6666 9.16667V14.1667" stroke="#B6B6B6" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/>
  ...
  
            {
              addedPizzas.map(obj => <CartItem 
                                          ...
                                          type={obj.type} 
                                          size={obj.size} 
                                          totalPrice = {items[obj.id].totalPrice}  //найди группу с total пиццами и вытащи оттуда price
